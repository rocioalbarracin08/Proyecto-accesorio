def test_toggle_empleado_activo(client):
   ...
   """
   #Loguea como dueño (usar credenciales existentes en la DB de test)
   ...
   #Obtiene la lista de empleados y buscar el empleado con id 1
   resp = client.get("/empleados/listar", headers={"Cookie": token_cookie})
   assert resp.status_code == 200
   empleados = resp.get_json()
   empleado = next((e for e in empleados if e.get('id_empleado') == 1), None)
   assert empleado is not None, "No se encontró empleado con id_empleado = 1"

   inicial_activo = empleado.get('activo')
   assert inicial_activo in (0, 1)

   #Llama al endpoint para alternar el estado
   patch_resp = client.patch('/empleados/desactivar/1', headers={"Cookie": token_cookie})
   ...
   empleado2 = next((e for e in empleados2 if e.get('id_empleado') == 1), None)
